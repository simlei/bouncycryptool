...
//trait ProjectFinder {
//  def find(refDir: File): Option[File]
//}
...
//
//  override def find(refDir: File): Option[File] = {
...
trait PresenceSpec[Pres <: FSPresence] { // specifies depending on a root directory
  ...
  def findRoot: Option[File] = find(searchRoot)
  def findOrBail(): File = find(searchRoot).getOrElse(sys.error(s"Spec: $this not found in $searchRoot."))

  def optionalVerifiedPresence(): Option[Pres] = find(searchRoot).flatMap(presenceConstructor(_).asOption)
...
case class ChildPatternSpec[Pres <: FSPresence](searchRoot: File, childPattern: String, presenceConstructor: File => Pres) extends PresenceSpec[Pres] {
  ...
  override def find(refDir: sbt.File) = (refDir * childPattern).get.headOption
}
...
case class SubpathSpec[Pres <: FSPresence](searchRoot: File, subpath: String, presenceConstructor: File => Pres) extends PresenceSpec[Pres] {
  ...
  override def find(refDir: sbt.File) = (searchRoot / subpath).get.headOption
}